...
import { upsertProfile, recordFetchCompletion, updateStatusDisplay, saveDatabaseToOPFS } from './database.js';
import { renderTable, renderProgress, updateButtons, scheduleTableRender, forceRenderTable } from './renderer.js';
...
export async function fetchAllProfiles(options = {}) {
  const { onItemDone, skipFinalMessage, manageState = true } = options;
  if (manageState) {
  ...
  try {
    ...
    if (state.fetchState !== 'stopped' && !skipFinalMessage) {
    ...
    if (manageState) {
      if (state.fetchState !== 'stopped') state.fetchState = 'idle';
      updateButtons();
      updateStatusDisplay();
      if (CONFIG.opfsEnabled) await saveDatabaseToOPFS(state.sqliteDB);
      forceRenderTable();
    }
  }
}
...
export async function fetchAllProfilesAndBlocked() {
  ...
  await fetchAllProfiles({ onItemDone, skipFinalMessage: true, manageState: false });
  if (state.fetchState === 'stopped') return;
  await fetchAllBlocked({ onItemDone, skipFinalMessage: true, manageState: false });
